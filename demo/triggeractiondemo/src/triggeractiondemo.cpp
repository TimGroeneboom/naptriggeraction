...
{    
    bool CoreApp::init(utility::ErrorState& error)
    {
        ...
        mUDPClient = mResourceManager->findObject<UDPClient>("UDPClient");
        if (!error.check(mUDPClient != nullptr, "unable to find UDPClient with name: %s", "UDPClient"))
            return false;

        mParameterGUI = mResourceManager->findObject<ParameterGUI>("ParameterGUI");
        if (!error.check(mParameterGUI != nullptr, "unable to find ParameterGUI with name: %s", "ParameterGUI"))
            return false;
    ...
    void CoreApp::update(double deltaTime)
    {
        ...
        {
            ImGui::InputText("UDP Message", &mMessage);
            if(ImGui::Button("UDP Send"))
            {
                mUDPClient->send(mMessage);
            }
        }
